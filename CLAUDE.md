...
- Question modes: rootMeaning, meaningRoot, wordRoot, buildWord (root + form → word), cloze (sentence
  blank), typeRoot, typeWord (dictation), hearWord, whichBinyan, guessWord; binyan lessons only:
  verbMeaning, spotBinyan. No odd-one-out (removed v0.8). `makeQuestion(…, fixed?)` pins the word.
  Example sentences live in `src/data/sentences/*.ts` keyed by the exact vocalized word;
  `src/data/sentences.test.ts` enforces the rules, including grammar guards: the key is the bank's
  form (3ms past verb / m.sg. adjective), so no such key after a pronoun or pointed plural noun of
  another gender/number (`SINGULAR_HEAD` names the right exceptions), the English opens with the
  Hebrew's pronoun, and a prefix glued to the key keeps the key's pointing right — never drop a
  dagesh to force a verbatim match; rewrite the sentence. Cloze tiles never offer a family word of
  the same kind as the key (it could fit too). All 1,012 were audited by hand for agreement
  (v1.1); a verb glossed "she"/"they" must have that shape (`roots.test.ts`). Conjugation is
  generated by rule (`src/lib/conjugate.ts`, strong roots only) — never stored.
- Typing: `KEY_ROWS` is the standard Israeli layout left to right with final letters (a test ties
  it to `QWERTY`); the `.kb` renders `dir="ltr"`. Answers keep final letters; compare with
  `sameLetters` (final = medial). Filling the last slot doesn't submit — check / Enter does.
  Dictation asks only single words (`typeWords`, ≤ `TYPE_MAX` letters).
- Binyan lessons: `Plan { kind: "binyan", binyan }` (`src/lib/binyan.ts`): 5 verbs met
  (verbMeaning), then used (spotBinyan / cloze / buildWord). `planRules().rootSrs` is false: they
  write word memory, XP, streak and gems but never a root's schedule, and log mistakes only for
  roots already seen. The binyan page is a `PAGE_VIEWS` entry (`ui.openBinyan`,
  `startBinyanLesson`); its summary returns there (`summaryExit`). Teaching content (spot, forms,
  uses) is in `src/data/binyanim.ts`.
- Feedback sheet: verdict pinned on top, Next pinned below, details scroll between. Never
  `autoFocus` inside a scroller (it scrolls the verdict away) — use `useFocusNoScroll`. English
  text with Hebrew words in it goes through `<Mixed text>` (each Hebrew word bidi-isolated).
...
- Six tabs (`SHELL_VIEWS`: home, path, shuk, bank, patterns, progress). Full-screen pages
  without a unit (`PAGE_VIEWS`: story, listen, notebook, league, binyan) open with
